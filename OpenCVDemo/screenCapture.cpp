...
void getHwndsByProcessName(const wchar_t* processName, vector<HWND>& list, bool visibleOnly)
{
    ...
    for (int i = 0; i < processes; i++)
    {
		...
		if (GetProcessImageFileNameW(process, nameBuffer, 1024))
		{
			printf("%d %ls\n",i, nameBuffer);
			/*if (i == 77) {
				printf("%d\n", wcscmp(nameBuffer, processName));
				printf("%d\n", wcsncmp(nameBuffer, processName, wcslen(processName)));
				printf("%d\n", wcslen(processName));
				printf("%d\n", wcsstr(nameBuffer, processName));

			}*/
			//if (wcsstr(nameBuffer, processName) - nameBuffer == wcslen(nameBuffer) - wcslen(processName)) 
			if (wcscmp(nameBuffer, processName) == 0 || wcsncmp(nameBuffer, processName, wcslen(processName)) == 0 || wcsstr(nameBuffer, processName) != 0)
			{
				//printf("%ls\n", nameBuffer);
